...
class account_view extends ObjectViewer
{
    public function printObject(): string
    {
        ...
        if (!empty($object->type_id)) {
            ...
            $account_type = $account_type->getById($object->type_id);
        ...
        $retval .= "<td data-label='Abertura'>{$object->open_date}</td>";
        $retval .= "<td data-label='Fecho'>{$object->close_date}</td>";
    ...
    public function printForm(): string
    {
        ...
        $account_type_view = ViewFactory::instance()->account_type_view($account_type);
        $tipo_opt = $account_type_view->getSelectFromList($account_type->getList(), isset($object->type_id) ? $object->type_id : null);
        ...
        $retval .= "<select class=\"date-fallback\" style=\"display: none\" name=\"aberturaAA\">" . Html::yearOptions(isset($object->open_date) ? substr($object->open_date, 0, 4) : null) . "</select>\r\n";
        $retval .= "<select class=\"date-fallback\" style=\"display: none\" name=\"aberturaMM\">" . Html::monthOptions(isset($object->open_date) ? substr($object->open_date, 5, 2) : null) . "</select>\r\n";
        $retval .= "<select class=\"date-fallback\" style=\"display: none\" name=\"aberturaDD\">" . Html::dayOptions(isset($object->open_date) ? substr($object->open_date, 8, 2) : null) . "</select>\r\n";
        $retval .= "<input class=\"date-fallback\" type=\"date\" name=\"abertura\" required value=\"" . (isset($object->close_date) ? $object->open_date : date("Y-m-d")) . "\">\r\n";
        $retval .= "</td>\r\n";
        $retval .= "<td data-label='Fecho'>\r\n";
        $retval .= "<select class=\"date-fallback\" style=\"display: none\" name=\"fechoAA\">" . Html::yearOptions(isset($object->close_date) ? substr($object->close_date, 0, 4) : null) . "</select>\r\n";
        $retval .= "<select class=\"date-fallback\" style=\"display: none\" name=\"fechoMM\">" . Html::monthOptions(isset($object->close_date) ? substr($object->close_date, 5, 2) : null) . "</select>\r\n";
        $retval .= "<select class=\"date-fallback\" style=\"display: none\" name=\"fechoDD\">" . Html::dayOptions(isset($object->close_date) ? substr($object->close_date, 8, 2) : null) . "</select>\r\n";
        $retval .= "<input class=\"date-fallback\" type=\"date\" name=\"fecho\" required value=\"" . (isset($object->close_date) ? $object->close_date : date("Y-m-d")) . "\">\r\n";
        $retval .= "</td>\r\n";
